...
export const ResourceApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * 国際化リソースを取得する
         * @summary 国際化リソース取得
         * @param {} UNKNOWN_PARAMETER_NAME ロケール
         * @param {*} [options] Override http request option.
         * @throws {RequiredError}
         */
        getI18n: async (UNKNOWN_PARAMETER_NAME: , options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            // verify required parameter 'UNKNOWN_PARAMETER_NAME' is not null or undefined
            assertParamExists('getI18n', 'UNKNOWN_PARAMETER_NAME', UNKNOWN_PARAMETER_NAME)
            const localVarPath = `/resource/i18n/{locale}`
                .replace(`{${"locale"}}`, encodeURIComponent(String(UNKNOWN_PARAMETER_NAME)));
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
            if (configuration) {
...
export const ResourceApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async getI18n(UNKNOWN_PARAMETER_NAME: , options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<Locale>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.getI18n(UNKNOWN_PARAMETER_NAME, options);
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
...
export const ResourceApiFactory = function (configuration?: Configuration, basePath?: string, axios?: AxiosInstance) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        getI18n(UNKNOWN_PARAMETER_NAME: , options?: RawAxiosRequestConfig): AxiosPromise<Locale> {
            return localVarFp.getI18n(UNKNOWN_PARAMETER_NAME, options).then((request) => request(axios, basePath));
        },
    };
};
...
export class ResourceApi extends BaseAPI implements ResourceApiInterface {
     ...
     */
    public getI18n(UNKNOWN_PARAMETER_NAME: , options?: RawAxiosRequestConfig) {
        return ResourceApiFp(this.configuration).getI18n(UNKNOWN_PARAMETER_NAME, options).then((request) => request(this.axios, this.basePath));
    }
}
