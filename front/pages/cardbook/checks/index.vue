        ...
        <div class="relative">
          ...
          <div class="flex flex-col gap-2 mx-4 mb-4">
            <div class="flex items-center justify-between">
              <span class="font-bold">{{ selectedResult.front }}</span>
              <span class="text-sm text-gray-500 ml-auto"
                >Checked: {{ formatToJST(selectedResult.checkedAt) }}</span
              >
            </div>
            <div class="text-sm text-gray-500">
              {{ cardbookNameMap.get(selectedResult.cardbookId ?? "") ?? "" }}
            </div>
...
import type { CardbookCheckResult } from "~/generated/api/client/api";
...
const fetchCheckResults = async () => {
  await withLoading(async () => {
    try {
      await cardbookStore.fetchCheckResults(
        undefined,
        checkedAtFrom.value,
        checkedAtTo.value,
  ...
  await withLoading(async () => {
    try {
      await Promise.all([
        ...
        cardbookStore.fetchCheckResults(
          undefined,
          checkedAtFrom.value,
          checkedAtTo.value,
          selectedSeverities.value,
...
const cardbookNameMap = computed(() => {
  const map = new Map<string, string>();
  (cardbookStore.cardbooks ?? []).forEach((cb) => {
    if (cb.cardbookId) map.set(cb.cardbookId, cb.name);
  });
  return map;
});
...
type CheckResultRow = CardbookCheckResult & {
  index: number;
  cardbookName: string;
...
const rows = computed<CheckResultRow[]>(() =>
  (cardbookStore.checkResults ?? []).map((r, i) => ({
    ...r,
    index: i + 1,
    cardbookName: cardbookNameMap.value.get(r.cardbookId ?? "") ?? "",
...
const selectedResult = ref<CardbookCheckResult | null>(null);
...
const onClickRow = (row: CheckResultRow) => {
  const found = cardbookStore.checkResults?.find(
    (r) => r.cardbookCheckResultId === row.cardbookCheckResultId,
  );
  selectedResult.value = found ?? null;
};

const onMarkAsChecked = async () => {
  if (!selectedResult.value?.cardbookCheckResultId) return;
  const loadingId = commonStore.addLoadingQueue();
  try {
    await cardbookStore.updateCheckResultStatus(
      selectedResult.value.cardbookCheckResultId,
      { status: "CHECKED" },
    );
    await fetchCheckResults();
