...
class CardbookQuizHistoryRepositoryTest :
    StringSpec({

        val enhanced = mockk<DynamoDbEnhancedClient>()
        val table = mockk<DynamoDbTable<CardbookQuizHistoryItem>>(relaxed = true)
        val repo = CardbookQuizHistoryRepository(enhanced, "cardbook-quiz-histories")
        val schema: TableSchema<CardbookQuizHistoryItem> = TableSchema.fromBean(CardbookQuizHistoryItem::class.java)
        ...
        "getByUserAndQuizHistoryId: PK+SK で1件返す" {
            every { enhanced.table("cardbook-quiz-histories", schema) } returns table
        ...
        "findByUser: userId で一覧を返す" {
            val page = mockk<Page<CardbookQuizHistoryItem>>()
            val iterable = mockk<PageIterable<CardbookQuizHistoryItem>>()
            every { enhanced.table("cardbook-quiz-histories", schema) } returns table
